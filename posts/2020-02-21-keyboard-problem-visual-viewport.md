...
> 在很多不便使用 100% 的情况下，我们会在 CSS 中使用 100vh 的的概念来代表视口高度，而这个高度在 Safari 中是表示工具栏自动收起时，视口的**最大高度**，因此会导致 100vh 高度的元素很可能已经溢出了 `html` 区域。
...
在键盘弹起的瞬间（`focus` 事件的下一个宏任务周期），我们可以从 `window.scrollY` 得知页面滚动的目标位置。很容易想到，此时我们可以通过 `window.scrollTo(0, 0)` 来恢复到原位置，但这样会导致页面整体向下瞬移，然后再逐渐移回到屏幕上。
...
如何实现一个 `fixed` 容器？关于这一点，也许有一部分 Web 开发者并不知情。在 Web 标准中，`fixed` 元素是始终相对于视口定位，没有任何一个元素能够改变它的定位方式；但事实上，问题却有些不同。
